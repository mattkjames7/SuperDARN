...
from ..Data.GetCellData import GetCellData
...
def RTI(Radar,Date,ut,Beams,Gates,Param='V',fig=None,maps=[1,1,0,0],
	...
	'''
	Plot RTI (range time intensity) for a set of radar cells.
	...
	if yaxistype.lower() in ['lon','longitude','maglon','mlon']:
		...
		yaxismid = np.arange(Beams.size)+0.5
	...
	for i in range(0,nCells):
		#get the data
		data = GetCellData(Radar,Date,ut,Beams[i],Gates[i])

...
c_long_ptr = np.ctypeslib.ndpointer(ctypes.c_long,flags="C_CONTIGUOUS")
...
def ctLong(x):
	'''
	Convert python Longeger to C
	
	'''
	return np.array([x]).flatten().astype('int64')[0]
	
	
def ctLongPtr(x):
	'''
	Convert python Longeger pointer to C
	
	'''
	return np.array([x]).flatten().astype('int64')
	
def ctLongPtrPtr(x):
	'''
	Convert python Longeger 2d array pointer to C
